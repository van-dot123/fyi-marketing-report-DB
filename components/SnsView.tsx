...
export default function SnsView({ posts, followers, ga4 }: { posts: SnsPostRow[]; followers: Record<SnsPlatform, number>; ga4: Ga4Day[] }) {
  ...
  const campaignDonut = useMemo(() => {
    ...
    for (const d of ga4Days) {
      ...
      const name = (d.campaign ?? "").trim();
      const lower = name.toLowerCase();
      // Drop noise rows entirely — no campaign signal.
      if (name === "" || lower === "(referral)" || lower === "(not set)" || lower === "(organic)" || lower === "(direct)") continue;
      // Numeric / unknown campaign IDs (e.g. 120…) → bucket into "Other".
      if (/^\d+$/.test(name)) {
        otherSum += d.sessions;
        continue;
      }
    ...
    otherSum += sorted.slice(8).reduce((s, [, v]) => s + v, 0);
    if (otherSum > 0) list.push({ name: "Other", value: otherSum });
    let p = 0;
    return list.map((c) => {
      let color: string;
      if (c.name === "Other") color = "#94a3b8";
      else {
