...
export interface SpendSessionsPoint {
  week: string;
  spend: number;
  sessions: number;
}
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Weekly spend vs. site sessions</p>
        </div>
      ...
      <div className="w-full" style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 20, right: 16, bottom: 0, left: 0 }}>
            ...
            <Bar yAxisId="left" dataKey="spend" name="Spend" fill="#7c3aed" radius={[4, 4, 0, 0]} barSize={36} isAnimationActive animationDuration={900} />
            <Line yAxisId="right" type="monotone" dataKey="sessions" name="Sessions" stroke="#0f172a" strokeWidth={2.5} dot={{ r: 3 }} isAnimationActive animationDuration={900} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </section>
