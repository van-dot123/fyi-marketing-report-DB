...
export default function PaidView({ meta }: { meta: MetaDay[] }) {
  ...
  const vm = useMemo(() => buildViewModel(meta, days, filter, metric, log), [meta, days, filter, metric, log]);
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            <div>
              ...
              <p style={{ fontSize: 12, color: "#94A3B8", marginTop: 3 }}>{vm.headerSub}</p>
            </div>
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18, flexWrap: "wrap", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap" }}>
              ...
              {vm.metricBtns.map((m) => (
                <div key={m.key} onClick={() => setMetric(m.key)} style={{ cursor: "pointer", padding: "5px 11px", borderRadius: 7, fontSize: 11, fontWeight: 700, background: m.bg, color: m.color, border: `1px solid ${m.border}`, transition: "all .12s" }}>
                  {m.label}
                </div>
              ))}
            </div>
          ...
          <div style={{ display: "flex", gap: 18, alignItems: "center", marginBottom: 6 }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <div style={{ width: 16, height: 0, borderTop: `2.5px solid ${vm.metricColor}` }} />
              <span style={{ fontSize: 11, color: "#64748B", fontWeight: 600 }}>{vm.metricLabel}</span>
            </div>
          </div>
          ...
          <div style={{ position: "relative" }}>
            <svg viewBox="0 0 940 264" style={{ width: "100%", height: "auto", overflow: "visible" }}>
              ...
              {vm.yRight.map((g, i) => (
                <text key={`r${i}`} x="886" y={g.y} textAnchor="start" dominantBaseline="middle" style={{ fontSize: 9, fill: vm.metricColor, fontWeight: 700, opacity: 0.7 }}>{g.label}</text>
              ))}
              {vm.optMarkers.map((o) => (
                <g key={`m${o.n}`}>
              ...
              {vm.metricHasLine && (
                <path d={vm.metricPath} fill="none" stroke={vm.metricColor} strokeWidth="2.25" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="6,4" />
              )}
              {vm.xTicks.map((t, i) => (
                <g key={`x${i}`}>
...
function buildViewModel(meta: MetaDay[], days: MetaDay[], filter: string, metric: Metric, log: LogEntry[]) {
  ...
  // KPIs
  const kpis = [
    { label: "Spend", value: fmt(aSpend), unit: "₩", sub: isAll ? `Across ${products.length} campaigns` : sel, color: "#2563EB", iconBg: "#DBEAFE", iconShape: "3px", cardBg: "#fff", cardBorder: "rgba(0,0,0,0.04)", valColor: "#0F172A" },
    { label: "Sessions", value: "—", unit: "", sub: "Awaiting Supabase (via UTM)", color: "#94A3B8", iconBg: "#F1F5F9", iconShape: "50%", cardBg: "#FAFAF9", cardBorder: "rgba(0,0,0,0.05)", valColor: "#CBD5E1" },
    ...
    { label: "CP Lead", value: aCpl > 0 ? fmt(aCpl) : "—", unit: "₩", sub: "Cost per lead", color: "#7C3AED", iconBg: "#EDE9FE", iconShape: "3px", cardBg: "#fff", cardBorder: "rgba(0,0,0,0.04)", valColor: "#0F172A" },
    { label: "CTR", value: aCtr.toFixed(2), unit: "%", sub: `${fmt(aClk)} link clicks`, color: "#D97706", iconBg: "#FEF3C7", iconShape: "50%", cardBg: "#fff", cardBorder: "rgba(0,0,0,0.04)", valColor: "#0F172A" },
  ];
  ...
  const METRICS: Record<Metric, { label: string; color: string; empty: boolean }> = {
    ...
    sessions: { label: "Sessions", color: "#DB2777", empty: true },
    installs: { label: "App installs", color: "#475569", empty: true },
  };
  const mInfo = METRICS[metric];
  const metricBtns = (Object.keys(METRICS) as Metric[]).map((k) => {
    const active = metric === k;
    return { key: k, label: METRICS[k].label, bg: active ? METRICS[k].color : "#fff", color: active ? "#fff" : "#64748B", border: active ? "transparent" : "rgba(0,0,0,0.08)" };
  });

  const spendVals = perDay.map((d) => d.spend);
  const metricVals = perDay.map((d) => {
    if (metric === "clicks") return d.clk;
    if (metric === "ctr") return d.imp > 0 ? (d.clk / d.imp) * 100 : 0;
    if (mInfo.empty) return 0;
    return d.conv;
  });
  ...
  const mMax = mInfo.empty ? 1 : niceMax(Math.max(...metricVals, 1));
  const yS = (v: number) => Y1 - (Y1 - Y0) * (v / sMax);
  const yM = (v: number) => Y1 - (Y1 - Y0) * (v / mMax);
  ...
  const metricPath = mInfo.empty ? "" : toPath(metricVals, yM);
  ...
  const yRight = mInfo.empty ? [] : Array.from({ length: ticks + 1 }, (_, i) => {
    const val = (mMax * i) / ticks;
    return { y: yM(val), label: metric === "ctr" ? `${val.toFixed(1)}%` : val >= 1000 ? `${(val / 1000).toFixed(0)}K` : fmt(val) };
  });
  ...
  const trendSub = `${isAll ? "All campaigns" : sel} · daily cost vs ${mInfo.label.toLowerCase()}`;
  const emptyMsg = metric === "sessions" ? "Sessions — awaiting Supabase data (matched via UTM)" : "App install tracking not yet wired";
  ...
  return {
    ...
    trendSub, metricBtns, metricColor: mInfo.color, metricLabel: mInfo.label,
    spendPath, spendArea, metricPath, metricHasLine: !mInfo.empty, yLeft, yRight, xTicks, metricEmpty: mInfo.empty, emptyMsg,
